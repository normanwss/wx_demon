...
public class ReadExcelUtils {
    ...
    public static List<String[][]> readExcel(String filePath) throws FileNotFoundException, FileFormatException {
        ...
        try {
            ...
            for (int numSheet = 0; numSheet < workbook.getNumberOfSheets(); numSheet++) {
                ...
                String[][] data = new String[lastRowIndex + 1][];
                // 读取首行 即,表头
                Row firstRow = sheet.getRow(firstRowIndex);
                if (firstRow == null) {
                    continue;
                }
                data[0] = new String[firstRow.getLastCellNum() + 1];
    ...
    public static void main(String[] args) {
        try {
            List<String[][]> result = new ReadExcelUtils().readExcel("C:\\Users\\chel\\Desktop\\6月改价要求v180531.xlsx");
            for (String[][] str : result){
                for (String[] str2: str){
                    String sqlTemp= "UPDATE `product` SET price = %s WHERE barcode = '%s';";
                    String sql = String.format(sqlTemp, str2[6], str2[3]);
